...
SELECT status , SUM(quantity * unit_price) AS TOTAL_SALES
FROM order_items OI
JOIN orders O 
ON (OI.order_id = O.order_id)
GROUP BY status
...
SELECT COUNT(employee_id) AS NUM_OF_EMP ,job_title
FROM employees
GROUP BY job_title;

-- 9. Total Number of Warehouses by Country
SELECT country_name,COUNT(*) AS TOTAL_Warehouses
...
-- how to select he mager and the emplyees work under this manager
...
-- how many proudect in each catogery
SELECT category_name,COUNT(*) AS TOTAL_Prodeuct
...
SELECT o.order_id,o.status,o.salesman_id,o.order_date,c.customer_id,c.name AS CUSTOMER_NAME
FROM orders o
join customers c
on(o.customer_id = c.customer_id);

-- 2. Find All Products with Their Categories
SELECT category_name,product_name AS Prodeuct_Name
FROM product_categories AS pc
JOIN products AS p
ON (p.category_id=pc.category_id);
...
SELECT c.customer_id, c.name , ct.first_name ,ct.phone
FROM customers c
JOIN contacts ct
ON (c.customer_id = ct.customer_id);

-- 5. Find Products Available in Specific Warehouses
SELECT p.product_id, p.product_name, w.warehouse_id, w.warehouse_name, i.quantity
FROM inventories i
